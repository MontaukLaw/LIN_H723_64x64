...
void sys_nvic_ex_config(GPIO_TypeDef *p_gpiox, uint16_t pinx, uint8_t tmode)
{
    ...
    for (pinpos = 0; pinpos < 16; pinpos++)
    {
        ...
        if (curpin == pos) /* 需要设置 */
        {
            ...
            if (tmode & 0x01)
                EXTI->FTSR1 |= 1 << pinpos; /* line bitx上事件下降沿触发 */
            if (tmode & 0x02)
                EXTI->RTSR1 |= 1 << pinpos; /* line bitx上事件上升降沿触发 */
        }
    }
}
...
uint8_t sys_clock_set(uint32_t pll1n, uint32_t pll1m, uint32_t pll1p, uint32_t pll1q, uint32_t pll2m, uint32_t pll2n, uint32_t pll2r, uint32_t pll2p)
{
    ...
    else
    {
        ...
        RCC->PLLCKSELR |= pll1m << 4;       /* DIVM1[5:0] = pllm, 设置PLL1的预分频系数 */
        RCC->PLL1DIVR |= (pll1n - 1) << 0;  /* DIVN1[8:0] = plln - 1, 设置PLL1的倍频系数, 设置值需减1 */
        RCC->PLL1DIVR |= (pll1p - 1) << 9;  /* DIVP1[6:0] = pllp - 1, 设置PLL1的p分频系数, 设置值需减1 */
        RCC->PLL1DIVR |= (pll1q - 1) << 16; /* DIVQ1[6:0] = pllq - 1, 设置PLL1的q分频系数, 设置值需减1 */
        ...
        RCC->PLLCKSELR |= pll2m << 12;      /* DIVM2[5:0] = 8, 设置PLL2的预分频系数 */
        RCC->PLL2DIVR |= (pll2n - 1) << 0;  /* DIVN2[8:0] = 440 - 1, 设置PLL2的倍频系数, 设置值需减1 */
        RCC->PLL2DIVR |= (pll2p - 1) << 9;  /* DIVP2[6:0] = 2 - 1, 设置PLL2的p分频系数, 设置值需减1 */
        RCC->PLL2DIVR |= (pll2r - 1) << 24; /* DIVR2[6:0] = 2 - 1, 设置PLL2的r分频系数, 设置值需减1 */
        ...
        // RCC Domain 1 configuration register,
...
void sys_stm32_clock_init(void)
{
    ...
    *((volatile uint32_t *)0x51008108) = 0x00000001; /* 设置AXI SRAM的矩阵读取能力为1 */
    ...
    // pll1m: 2 pll1n: 44, pll1p: 1, pll1q: 2
    // pll2m: 5 pll2r: 2 pll2n:160, pll2p:2
    sys_clock_set(44, 2, 1, 2, 5, 160, 2, 2); /* 设置时钟 */
